...
include_once("function/function_bastp.php");
?>
    ...
    <td width="1028" height="173" colspan="3" align="left" valign="top" bgcolor="#FFFFFF"><table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
      ...
      <tr>
        ...
        <th class="_css_font_default_11" scope="col"><div align="left">
          <div align="left"><strong>Dokumen Kontrak - BA Serah Terima Pekerjaan</strong></div></div></th>
        <th class="_css_font_default_11" scope="col">&nbsp;</th>
      </tr>
      <tr>
        <th scope="col">&nbsp;</th>
		...
		<table id="example" class="display" cellspacing="0" width="100%">
				<thead class="_css_font_default_11_b">
					<tr>
                        ...
                        <th><div align='center'>Nomor bastp</div></th>
 						<th>Tanggal  </th>
                        <th>Direksi Pekerjaan</th>
                        <th>Direksi Vendor</th>
                        <th>File Kontrak bastp</th>                        
						<th>Bidang</th>
                        <th>User Input</th>                        
					</tr>
				</thead>
				...
				<tbody class="_css_font_default_11_b">
					...
					$bastp = select_bastp();
					$nomor=1;
					for ($i=0;$i<count($bastp);$i++) {
                    
					$current_id_bastp = $bastp[$i][0];
					$current_id_spk = $bastp[$i][1];
					$current_id_spk_angsuran = $bastp[$i][2];
					$current_nomor_bastp = $bastp[$i][3];
					$current_tanggal_bastp = $bastp[$i][4];
					$current_direksi_pekerjaan = $bastp[$i][5];
					if (trim($current_direksi_pekerjaan=="")) { $current_direksi_pekerjaan="Direksi Pekerjaan not found";}


					$current_direksi_vendor = $bastp[$i][6];
					if (trim($current_direksi_vendor=="")) { $current_direksi_vendor="Direksi Vendor not found";}

					
					$current_file_bastp = $bastp[$i][7];
					$current_keterangan = $bastp[$i][8];
					$current_unit_bidang = $bastp[$i][9];
					$current_user_input = $bastp[$i][10];
					$current_user_input_date = $bastp[$i][11];
					$current_info1 = $bastp[$i][12];
					$current_info2 = $bastp[$i][13];
					$current_info3 = $bastp[$i][14];
					$current_info4 = $bastp[$i][15];
					$current_info5 = $bastp[$i][16];
					$current_info6 = $bastp[$i][17];
					$current_info7 = $bastp[$i][18];
					$current_info8 = $bastp[$i][19];
					$current_info9 = $bastp[$i][20];
					$current_info10 = $bastp[$i][21];
					
					echo "<tr>";
						echo "<td>".$nomor."</td>";	
                        echo "<td>".$current_id_spk."</td>";
						echo "<td><div align='right'>".strtoupper($current_nomor_bastp)."</div></td>";
                        echo "<td>".strtoupper($current_tanggal_bastp)."</td>";
                        
                        echo "<td>".strtoupper($current_direksi_pekerjaan)."</td>";
                        echo "<td>".strtoupper($current_direksi_vendor)."</td>";
                        echo "<td>".$current_file_bastp."</td>";
